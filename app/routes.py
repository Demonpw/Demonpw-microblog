...
def post():
	...
	if form.validate_on_submit():
		...
		article=Article(title=form.title.data,body = form.body.data,create_time = datetime.now(),pic_path='static/assets/img/'+form.file.data.filename,category_id=cate.id)
		db.session.add(article)
		db.session.commit()
		flash('上传成功！')
		return redirect(url_for('index'))
...
@app.route('/reply/<int:article_id>/<int:user_id>/<int:user_to_id>', methods=['GET', 'POST'])
@csrf.exempt
def reply(article_id,user_id,user_to_id):
	title="回复评论"
	form = CommentForm()
	article = Article.query.filter_by(id=article_id).first()
	reply_to = Post.query.filter_by(user_id=user_id).first()
	if request.method == 'POST':
		post_username=form.username.data
		post_text=form.post.data
		post_email=form.email.data
		...
		user2=User.query.filter_by(email=post_email).first()
		if user1 == user2 and user1 is not None and user2 is not None:
			post=Post(user_id=user1.id,content=post_text,is_appear=post_is_appear,article_id=article.id,reply_id=reply_to.id,user_to_id=user_to_id)
			db.session.add(post)
			db.session.commit()
			flash('提交成功')
			...
			db.session.add(user)
			db.session.commit()
			post=Post(user_id=user.id,content=post_text,is_appear=post_is_appear,article_id=article.id,reply_id=reply_to.id,user_to_id=user_to_id)
			db.session.add(post)
			db.session.commit()
			flash('提交成功')
			return redirect(url_for('article',id=article_id))
			...
			flash('昵称或者邮箱已经被使用！')
			return redirect(url_for('reply',article_id=article_id,user_id=user_id,user_to_id=user_to_id))
	return render_template('reply.html',title=title,article=article,form=form)

@app.route('/cmtmanage', methods=['GET', 'POST'])
@csrf.exempt
def cmtmanage():
	title="评论管理"
	comments=Post.query.order_by(Post.timestamp.desc())
	return render_template('cmtmanage.html',title=title,comments=comments)